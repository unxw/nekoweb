...
function deviceCheck() {
  ...
  if (mobile) {
    window.alert(
      "youre on a mobile device, im lazy and have yet to fix support so the site might not work...!!",
    );
  }
...
async function ps2thingy() {
  //async so that i can acutaly use await
...
async function getInfo() {
  ...
  const url = "https://nekoweb.org/api/site/info/nixora.nekoweb.org";
  try {
    ...
    info.textContent = "v" + result.updates / 10;
  } catch (error) {
    console.error(error.message);
    info.addEventListener("onclick", () => console.log("trying to get site info again..."));
    info.style.fontSize = "3vh";
    info.textContent = "couldnt get site stats... try reloading :<";
  }
}
...
function confirmation() {
  ...
  console.log(
    "%call scripts loaded okay!🌸  ...probably?\n(im a shitty coder sorry)\n\nwindow location:" +
      wlocation,
    "color: #ffc4f3; font-size: 20px;",
  ); //ts doesnt even check to see if it loaded just pray for the best ngl
}
